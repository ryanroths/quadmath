...
function lintCli(text, m) {
  const warns = [];
  const kind = m.dScheme && m.dScheme.kind;
  const cur = m.dScheme ? m.dScheme.values : null;
  const sets = {};
  for (const mt of text.matchAll(/^\s*set\s+([a-z0-9_]+)\s*=\s*(-?\d+)/gim)) sets[mt[1].toLowerCase()] = +mt[2];
  const keys = Object.keys(sets);
  if (kind === 'd_max' && keys.some(k => k.startsWith('d_min_'))) {
    warns.push('this firmware has no d_min_* — it uses d_<axis> as the floor and d_max_<axis> as the ceiling. That line will be rejected by the CLI.');
  }
  if (kind === 'd_min' && keys.some(k => k.startsWith('d_max_'))) {
    warns.push('this firmware has no d_max_* — it uses d_min_<axis> as the floor and d_<axis> as the ceiling.');
  }
  ['roll', 'pitch', 'yaw'].forEach((ax, i) => {
    let floor, ceil;
    if (kind === 'd_max') { floor = sets['d_' + ax] ?? cur?.floor?.[i]; ceil = sets['d_max_' + ax] ?? cur?.ceiling?.[i]; }
    else if (kind === 'd_min') { floor = sets['d_min_' + ax] ?? cur?.floor?.[i]; ceil = sets['d_' + ax] ?? cur?.ceiling?.[i]; }
